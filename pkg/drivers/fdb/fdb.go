...
type FDB struct {
	...
	// watch delivery matters: the apiserver's CRD finalizer loses an upstream
	// conflict-retry race when informer listers lag more than a few ms behind
	// writes, leaving CRD deletion stalled until the next informer resync.
	pollKick chan struct{}
}

// kickPoll nudges the poll loop without blocking; a full channel already
// guarantees a wakeup.
func (f *FDB) kickPoll() {
	select {
	case f.pollKick <- struct{}{}:
	default:
	}
}
